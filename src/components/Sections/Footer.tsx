...
const ListItem = ({ Icon, ...props }: ItemProps) => {
    ...
    return (
      <div
        className='flex flex-col items-center w-[80px] relative group'
        onClick={props.onClick}
      >
        <Icon fill='#fff' stroke='#fff' color='#fff' />
        <span
          className={`w-full h-[6px] bg-primary mt-6 absolute -bottom-6 transition-opacity duration-300 ${
...
export function Footer() {
  ...
  return (
    <div className='w-full flex flex-col xl:flex-row justify-center h-[40vh] min-h-[400px] bg-[#090D1F] overflow-hidden'>
        ...
        <div className='relative h-full min-h-[300px] w-full xl:w-[60%] bg-secondary flex flex-col justify-center items-center gap-16'>
            ...
            <ListItem
                Icon={GithubIcon}
                isActive={activeIndex === 0} // Verifica se é o item ativo
                onClick={() => handleItemClick(0, "@WesleyFerreira97")} // Define qual item foi clicado
            />
            <ListItem
                Icon={LinkedinIcon}
                isActive={activeIndex === 1}
                onClick={() => handleItemClick(1, "@WesleyFerreira97")}
            />
            <ListItem
                Icon={MailIcon}
                isActive={activeIndex === 2}
                onClick={() => handleItemClick(2, 'Wesleyferreira97@outlook.com')}
            />
            <ListItem
                Icon={CVIcon}
                isActive={activeIndex === 3}
                onClick={() => handleItemClick(3, "Download Currículo")}
            />
            </div>
            <div className='max-xl:absolute max-xl:bottom-0 w-full flex justify-center items-center bg-[#191B2F] text-white text-center'>
                <span className='p-2 flex font-primary text-[#ABABAB] text-sm font-semibold'>
                    {currentBarText}
                </span>
            </div>
        </div>
